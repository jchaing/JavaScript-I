...
console.log(
  `The last car in inventory is a ${lastCar.car_make} ${lastCar.car_model}`
);
...
let carModels = inventory.sort(function(a, b) {
  let carA = a.car_model.toLowerCase();
  let carB = b.car_model.toLowerCase();
  if (carA < carB) {
    return -1;
  }
  if (carA > carB) {
...
let carModels_arrow = inventory.sort((a, b) => {
  let carA = a.car_model.toLowerCase();
  let carB = b.car_model.toLowerCase();
  return carA < carB ? -1 : carA > carB ? 1 : 0;
});

console.log(carModels_arrow);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
function yearsOnly(inv) {
  for (let i = 0; i < inv.length; i++) {
    carYears.push(inv[i].car_year);
  }
}
yearsOnly(inventory);
carYears = carYears.sort((a, b) => a - b);

console.log(carYears);

// forEach
let carYears_forEach = [];
inventory.forEach(x => carYears_forEach.push(x.car_year));
carYears_forEach = carYears_forEach.sort((a, b) => a - b);

console.log(carYears_forEach);

// map
let carYears_map = inventory
  .map(car => car.car_year)
  .sort((year1, year2) => year1 - year2);

console.log(carYears_map);
...
console.log(`There were ${oldCars.length} cars made before the year 2000`);

// filter
let oldCars_filter = [];
carYears.filter(year => (year < 2000 ? oldCars_filter.push(year) : 0));

console.log(
  `There were ${oldCars_filter.length} cars made before the year 2000`
);
...
// forEach
let BMWAndAudi_forEach = [];
inventory.forEach(car =>
  car.car_make === 'BMW' || car.car_make === 'Audi'
    ? BMWAndAudi_forEach.push(car)
    : 0
);

console.log(JSON.stringify(BMWAndAudi_forEach));

// filter
let BMWAndAudi_filter = inventory.filter(
  car => car.car_make === 'BMW' || car.car_make === 'Audi'
);

console.log(JSON.stringify(BMWAndAudi_filter));
